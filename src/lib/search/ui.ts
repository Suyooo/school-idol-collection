...
export type SearchUiOptions = {
    ...
    songPiecesCool?: string, songPiecesCoolMod?: NumberQueryMod,
    songPiecesAll?: string, songPiecesAllMod?: NumberQueryMod
};
...
const mapSelectInputReverse: Map<string, keyof SearchUiOptions> = new Map(
    [...mapSelectInput.entries()]
        .map(([name, {urlParamOptions}]) =>
            urlParamOptions.map(param => (<[string, keyof SearchUiOptions]>[param, name]))).flat()
);

const mapTextInput: Map<keyof SearchUiOptions, { urlParam: string, condition?: (options: SearchUiOptions) => boolean }>
    = new Map([
...
const mapTextInputReverse: Map<string, keyof SearchUiOptions> = new Map(
    [...mapTextInput.entries()]
        .map(([name, {urlParam}]) => (<[string, keyof SearchUiOptions]>[urlParam, name])));

const mapNumberInput: Map<keyof SearchUiOptions, { urlParam: string, condition?: (options: SearchUiOptions) => boolean }>
    = new Map([
