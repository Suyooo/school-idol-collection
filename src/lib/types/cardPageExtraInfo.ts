...
async function addNeighborCards(DB: Sequelize, card: Card & CardPageExtraInfo<boolean, true>) {
	card.neighbors = {
		prev:
			(await DB.m.Card.withScope(["viewCardNoOnly", { method: ["filterBefore", card.cardNo] }]).findOne())?.cardNo ??
			null,
		next:
			(await DB.m.Card.withScope(["viewCardNoOnly", { method: ["filterAfter", card.cardNo] }]).findOne())?.cardNo ??
			null,
	};
