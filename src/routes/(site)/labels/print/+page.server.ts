...
export const actions = {
    default: async ({ request, locals, fetch }) => {
        ...
        const enteredCardNos = (data.get("cardNos") as string)
            .toUpperCase()
            .split(/[ 　\n\r,、]/)
            .filter((s) => s.length > 0);
                    ...
                    if (!r.ok) {
                        ...
                        return r.json().then((c) => {
                            ...
                            if (cardIsMember(c) && cardHasGroup(c)) {
                                allRequiredGroupMembers.push(
                                    ...c.member.group.expectedMemberIds
                                        .split("|")
                                        .filter((id) => id !== "")
                                        .map((x) => parseInt(x))
                                );
        ...
        await Promise.all(
            allRequiredGroupMembers
                ...
                .map(async (cardId) => {
                    ...
                    await fetch(`/json/card/${cardNo}/preparse`)
                        .then((r) => r.json())
                        .then((c) => {
                            byCardNo[c.cardNo] = c;
                            byCardId[c.id] = c.cardNo;
        ...
        const duplicateCardNos = Object.keys(cardNoCounts)
            .filter((cardNo) => cardNoCounts[cardNo] > 1)
            .map((cardNo) => `${cardNo} (x${cardNoCounts[cardNo]})`);

        return {
            width: data.get("width"),
            height: data.get("height"),
            padding: data.get("padding"),
            cardNos: cardNosAfterFilter,
            byCardNo,
            byCardId,
            invalidCardNos,
            filteredCardNos,
