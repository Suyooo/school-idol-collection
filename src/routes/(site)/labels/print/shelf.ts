export default class Shelving<T> {
    ...
    public add(newItemKey: T, newItemWidth: number, newItemHeight: number) {
        ...
        while (true) {
            ...
            if (best === undefined || (previousLoopBestEmptyArea !== undefined && best.toReplaceOrNull !== null && best.emptyArea >= previousLoopBestEmptyArea)) {
...
class Shelf<T> {
    ...
    public getLowestEmptyAreaFor(newItemWidth: number, newItemHeight: number) {
        ...
        if (this.freeWidth >= newItemWidth) {
            lowestEmptyArea = calcEmptyArea(this.maxWidth, [...this.shelf, {width: newItemWidth, height: newItemHeight}]);
        ...
        for (const item of this.shelf) {
            if ((item.width < newItemWidth
                ...
                const testEmptyArea = calcEmptyArea(this.maxWidth, [...this.shelf.filter(i => i !== item), { width: newItemWidth, height: newItemHeight }]);
